...
namespace Nez
{
	public class DefaultBatcher : Batcher
	{
		/// <summary>
		/// Matrix to be used when creating the projection matrix
		/// </summary>
		/// <value>The transform matrix.</value>
		public override Matrix TransformMatrix { get => _transformMatrix;
		...
		#if FNA
		static DefaultBatcher() => UseFnaHalfPixelMatrix = true;
		#endif

		public DefaultBatcher(GraphicsDevice graphicsDevice) : base(graphicsDevice)
		{
		...
		public override void SetIgnoreRoundingDestinations(bool shouldIgnore)
		{
		...
		public override void Begin(BlendState blendState, SamplerState samplerState, DepthStencilState depthStencilState,
		...
		public override void Draw(Texture2D texture, Rectangle destinationRectangle)
		{
			CheckBegin();
		...
		public override unsafe void DrawRaw(Texture2D texture, Vector3[] verts, Vector2[] textureCoords, Color[] colors)
		{
		...
		public override unsafe void DrawRaw(Texture2D texture, Vector3[] verts, Vector2[] textureCoords, Color color)
		{
			Insist.IsTrue(verts.Length == 4, "there must be only 4 verts");
			Insist.IsTrue(textureCoords.Length == 4, "there must be only 4 texture coordinates");
		...
		public override unsafe void FlushBatch()
		{
			if (_numSprites == 0)
				return;
		...
		public override void EnableScissorTest(bool shouldEnable)
		{
